...
def department():
    ...
    showfields = (("dept_name", _("Department")),
        ("manager", _("Manager")),
        ("parent_department", _("Parent Department")))
    return render_template("list.html", folder="hr", link="hr.department",
        showfields=showfields, table=department.joinall())
...
def department_edit(id):
    ...
    form.parent_id.choices.extend([(g.id, g.dept_name) for g in
        Department.query.filter_by(active=True).order_by('dept_name')])
...
def job():
    job = Job()
    showfields = (("job_name", _("Job")),
        ("department", _("Department")),
        ("description", _("Description")))
    return render_template("list.html", folder="hr", link="hr.job",
        showfields=showfields, table=job.joinall())
...
def job_edit(id):
    ...
    form.department_id.choices.extend([(g.id, g.dept_name) for g in
        Department.query.filter_by(active=True).order_by('dept_name')])
